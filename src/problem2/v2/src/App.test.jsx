...
describe("Currency Swap V2", () => {
  ...
  it("switches the send and receive tokens without changing the amount", async () => {
    ...
    expect(screen.getByLabelText(/token to send/i).value).toBe("USDC");
    expect(screen.getByLabelText(/token to receive/i).value).toBe("ETH");
    expect(screen.getByLabelText(/amount to send/i).value).toBe("1");
  });
});
